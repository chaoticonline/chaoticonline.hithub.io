...
if(index<0){index=0;}
items[index].focus();};_createClass(Dropdown,null,[{key:"VERSION",get:function get(){return VERSION;}},{key:"Default",get:function get(){return Default;}},{key:"DefaultType",get:function get(){return DefaultType;}}]);return Dropdown;}();$$$1(document).on(Event.KEYDOWN_DATA_API,Selector.DATA_TOGGLE,Dropdown._dataApiKeydownHandler).on(Event.KEYDOWN_DATA_API,Selector.MENU,Dropdown._dataApiKeydownHandler).on(Event.CLICK_DATA_API+" "+Event.KEYUP_DATA_API,Dropdown._clearMenus).on(Event.CLICK_DATA_API,Selector.DATA_TOGGLE,function(event){event.preventDefault();event.stopPropagation();Dropdown._jQueryInterface.call($$$1(this),'toggle');}).on(Event.CLICK_DATA_API,Selector.FORM_CHILD,function(e){e.stopPropagation();});$$$1.fn[NAME]=Dropdown._jQueryInterface;$$$1.fn[NAME].Constructor=Dropdown;$$$1.fn[NAME].noConflict=function(){$$$1.fn[NAME]=JQUERY_NO_CONFLICT;return Dropdown._jQueryInterface;};return Dropdown;}($,Popper);return Dropdown;})));/*!
* Bootstrap modal.js v4.2.1 (https://getbootstrap.com/)
* Copyright 2011-2018 The Bootstrap Authors (https://github.com/twbs/bootstrap/graphs/contributors)
* Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)
*/(function(global,factory){typeof exports==='object'&&typeof module!=='undefined'?module.exports=factory(require('jquery'),require('./util.js')):typeof define==='function'&&define.amd?define(['jquery','./util.js'],factory):(global.Modal=factory(global.jQuery,global.Util));}(this,(function($,Util){'use strict';$=$&&$.hasOwnProperty('default')?$['default']:$;Util=Util&&Util.hasOwnProperty('default')?Util['default']:Util;function _defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}
function _createClass(Constructor,protoProps,staticProps){if(protoProps)_defineProperties(Constructor.prototype,protoProps);if(staticProps)_defineProperties(Constructor,staticProps);return Constructor;}
function _defineProperty(obj,key,value){if(key in obj){Object.defineProperty(obj,key,{value:value,enumerable:true,configurable:true,writable:true});}else{obj[key]=value;}
return obj;}
function _objectSpread(target){for(var i=1;i<arguments.length;i++){var source=arguments[i]!=null?arguments[i]:{};var ownKeys=Object.keys(source);if(typeof Object.getOwnPropertySymbols==='function'){ownKeys=ownKeys.concat(Object.getOwnPropertySymbols(source).filter(function(sym){return Object.getOwnPropertyDescriptor(source,sym).enumerable;}));}
...
this._element.style.display='block';this._element.removeAttribute('aria-hidden');this._element.setAttribute('aria-modal',true);this._element.scrollTop=0;if(transition){Util.reflow(this._element);}
...
_createClass(Location,[{key:"init",value:function init(){var options=this.getOptions();this.data=options.data;this.url=options.url;this.results=$('#results');}},{key:"build",value:function build(){$('#templates .locations').clone().insertAfter(this.results.find('h4'));}},{key:"parse",value:function parse(){var name=this.data.find('h3').first().text().trim();var picture=this.data.find('.picture:not(.big) img').first().attr('src');var region=this.data.find('.region').first().text().trim();var rarity=this.data.find('.rarity').text().trim();var initiative=this.data.find('.initiative').first().text().trim();$('<tr/>').append($('<td/>',{'class':'name'}).html($('<a/>',{'href':this.url}).text(name))).append($('<td/>',{'class':'picture hidden'}).text(picture)).append($('<td/>',{'class':'rarity'}).text(rarity)).append($('<td/>',{'class':'region'}).text(region)).append($('<td/>',{'class':'initiative'}).text(initiative)).append($('<td/>',{'class':'link','colspan':'2'}).html($('<a/>',{'href':this.url}).attr('target','_blank').text(this.results.find('table').attr('data-more-text')))).appendTo(this.results.find('tbody'));}}]);return Location;}(Core);"use strict";function _typeof(obj){if(typeof Symbol==="function"&&typeof Symbol.iterator==="symbol"){_typeof=function _typeof(obj){return typeof obj;};}else{_typeof=function _typeof(obj){return obj&&typeof Symbol==="function"&&obj.constructor===Symbol&&obj!==Symbol.prototype?"symbol":typeof obj;};}return _typeof(obj);}
function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}
function _defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}
function _createClass(Constructor,protoProps,staticProps){if(protoProps)_defineProperties(Constructor.prototype,protoProps);if(staticProps)_defineProperties(Constructor,staticProps);return Constructor;}
function _possibleConstructorReturn(self,call){if(call&&(_typeof(call)==="object"||typeof call==="function")){return call;}return _assertThisInitialized(self);}
...
parser.build();$.each(data,function(offset,current){var url=$(current).find('a').attr('href').replace(/\/?$/,'/');_this2.load(url).done(function(response){parser.setOptions({'data':$(jQuery.parseHTML(response)),'url':url},true).parse();});});deferred.resolve();}).fail(function(){_this2.messages.find('p.error').removeClass('hidden');deferred.reject();});},function(deferred){var results=_this2.results.find('table').footable();results.on('click','.name a',function(event){if(_this2.isMobile())event.preventDefault();});results.on('expand.ft.row',function(e,ft,row){var cell=$(row.$el).find('td.picture');cell.html(jQuery('<img/>',{'class':'picture'}).attr('src',cell.text())).removeClass('hidden');});results.on('ready.ft.table',function(){deferred.resolve();});},function(deferred){$('#pagination .pagination li').each(function(){$(this).addClass('page-item').find('a').addClass('page-link');});deferred.resolve();}).done(function(){_this2.results.addClass('hidden').removeClass('hidden');_this2.messages.addClass('hidden');_this2.loading.addClass('hidden');$('#pagination').removeClass('hidden');}).fail(function(){_this2.reset();_this2.messages.removeClass('hidden');});}},{key:"reset",value:function reset(){this.results.addClass('hidden').children().not('h4, #pagination').remove();this.messages.addClass('hidden');this.loading.addClass('hidden');$('#pagination').empty();}}]);return Search;}(Core);new Search();(function(){'use strict';if(navigator.userAgent.match(/IEMobile\/10\.0/)){var msViewportStyle=document.createElement('style');msViewportStyle.appendChild(document.createTextNode('@-ms-viewport{width:auto!important}'));document.head.appendChild(msViewportStyle);}}());$(document).ready(function(){updateGDPRConsentStatus();var $serverSelection=$('#server-selection');var $loading=$('.loading-icon');var hash=window.location.hash;var tab=$('#servers a[href="'+hash+'"]');if(hash&&tab.length>0){tab.tab('show');$serverSelection.hide();$loading.removeClass('hidden');var $provider=$(hash).find('.player');$('<iframe/>',{'src':$provider.parent().attr('data-url')}).attr({'title':$('h3').text(),'allow':'encrypted-media','allowfullscreen':'','mozallowfullscreen':'','webkitAllowFullScreen':'','scrolling':'no'}).appendTo($provider);$provider.find('iframe').on('load iframeready',onAfterLoad(hash));}
$('#servers a').on('click',function(){$loading.removeClass('hidden');var $provider=$(this.hash).find('.player');if($provider.find('iframe').length==0){$('<iframe/>',{'src':$provider.parent().attr('data-url')}).attr({'allowfullscreen':'','mozallowfullscreen':'','webkitAllowFullScreen':'','scrolling':'no'}).appendTo($provider);}
$provider.find('iframe').on('load iframeready',onAfterLoad(this));});$('#sharing .social-medias li a').on('click',function(e){var $this=$(this);e.preventDefault();popup($this.attr('data-href'),$this.data('title')||'',$this.data('width')||screen.width,$this.data('height')||screen.height);});$('#sharing').on('show.bs.modal',function(){$('#sharing .link .copy').removeClass('hidden').next('.copied').addClass('hidden');});$('#sharing .link .copy').on('click',function(){const $this=$(this);clipboard.writeText($('#sharing .link input').val()).then(function(){$this.addClass('hidden').next('.copied').removeClass('hidden');});});$('footer.gdpr .accepted').on('click',function(){Cookies.set('GDPR','1',{expires:365});updateGDPRConsentStatus();window.location.reload(false);});$('footer.gdpr .declined').on('click',function(){Cookies.set('GDPR','0',{expires:365});updateGDPRConsentStatus();});(function(){var timeout;$('abbr').on('click',function(){const $abbr=$('#abbr').empty();const $this=$(this);const title=$this.html();const expandedTerm=$this.attr('title');const wpm=(Math.round((60000*expandedTerm.split(' ').length)/125));$abbr.append($('<span/>',{'class':'title'}).html(title)).append($('<p/>').html(expandedTerm)).addClass('shown');if(timeout)window.clearTimeout(timeout);timeout=window.setTimeout(function(){$abbr.removeClass('shown');},(wpm>=3000?wpm:3000));});})();});$(window).on('scroll',function(){var $actions=$('.actions');var $data=$('.card-data');if($data.length>0){if($(window).scrollTop()<$data.first().offset().top){$actions.first().append($actions.last().children());}else{$actions.last().append($actions.first().children());}}});function onAfterLoad(chosen){$(chosen).tab('show');$serverSelection.hide();window.location.hash=chosen.hash||chosen;$('html, body').scrollTop(0);}
function updateGDPRConsentStatus(){if((Cookies.get('GDPR')||0)){$('footer.gdpr').addClass('hidden');}else{$('footer.gdpr').removeClass('hidden');}}
