/**
 * Card Search All Cards Data Parser | themes\chaotic\src\js\cards\parsers\All.js
 *
...
class All extends Core {
     ...
     */
    build() {
        $( '#templates .all' ).clone().insertAfter( this.results.find( 'h4' ) );
    }
    ...
    parse() {
        ...
        let type      = this.data.find( 'h2' ).first().text().trim();

        /**
         * @internal
         *
         * Some Cards have variations of themselves, so let's pick-up data
        ...
        .append( $( '<td/>', { 'class': 'type' } ).text( type ) )
        .append( $( '<td/>', { 'class': 'rarity' } ).text( rarity ) )
        .append(
            $( '<td/>', { 'class': 'hidden-sm-up link' }).html(
